...
def getleechinfo(from_user):
    ...
    text = f"⚙️<u>Leech Settings for <a href='tg://user?id={user_id}'>{name}</a></u>\n"\
...
def setLeechType(update, context):
    ...
    if user_id != int(data[1]):
        ...
        query.answer(text="Your File Will Deliver As Media🎞️!", show_alert=True)
        editLeechType(message, query)
    elif data[2] == "thumb":
...
def setThumb(update, context):
    ...
    if reply_to is not None and reply_to.photo:
        ...
        msg = f"🖼️ Custom thumbnail saved for {update.message.from_user.mention_html(update.message.from_user.first_name)}."
        sendMessage(msg, context.bot, update.message)
    else:
        sendMessage("Reply to a photo to save custom thumbnail🖼️.", context.bot, update.message)
